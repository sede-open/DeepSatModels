...
def get_environment(ml_client: MLClient, name: str, version: int) -> Environment:
    ...
    try:
        ...
        env = Environment(
            ...
            conda_file='environment/deepsatmodels_env.yml',
        )
        # build_context = BuildContext(path="environment")
        # env = Environment(
        #     build=build_context,
        #     name=name,
        #     description='DeepSatModels environment'
        # )
...
def get_job_args(env, data_folder):
    ...
    # For output description, see https://github.com/Azure/azure-sdk-for-python/blob/629a2ab4e37883d5810ff2275b2850276a4654d7/sdk/ml/azure-ai-ml/azure/ai/ml/entities/_inputs_outputs/output.py#L20
    # job_args["outputs"] = dict(
    #     outputs=Output(
    #         type=AssetTypes.URI_FILE,
    #         mode=InputOutputModes.UPLOAD,
    #         path="wasbs://naturevers@semsegstorage.blob.core.windows.net/naturevers/v2/zone_{}/zone_{}_raw.gpkg",
    #     )
    # )

    # job_args["command"] = """python infer_rod.py \
    #                             --cfg ${{inputs.cfg}} \
    #                             --model_ckpt ${{inputs.model_ckpt}} \
    #                             --data_folder ${{inputs.data_folder}} \
    #                             --filtering_data_folder ${{inputs.filtering_data_folder}} \
    #                             --zone ${{inputs.zone}} \
    #                             --output_folder ${{outputs.outputs}} \
    #                             """
